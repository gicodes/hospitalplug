...
import { mockHospitals } from '@/app/components/dashboard/admin/mock-data';
...
export default function Geolocation() {
  ...
  const fetchNearbyHospitals = useCallback(async () => {
    ...
    if (position.lat && position.lng) {
      ...
      setHospitals(res.data.data || mockHospitals);
    // */
    }
  }, [filters, position]);

  useEffect(() => {
    if (navigator.geolocation && !searchForSomeoneElse) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          showAlert(
            'info',
            'Unable to detect your location automatically. Please enter a location manually to find nearby hospitals.'
          );
        }
      );
    }
  }, [searchForSomeoneElse, showAlert, fetchNearbyHospitals]);
  ...
  return (
    <div className={styles.discover}>
      ...
      <div className={styles.nearby}>
        ...
        <p>
          Location results may vary — adjust your filters for the best match.
        </p>
      </div>
      ...
      <div className={styles.altSearch}>
        ...
        >
          {searchForSomeoneElse
            ...
            : 'Searching for someone else? Use a precise location'}
        </span>
      ...
      <div className={styles.hospitalView}>
        <ul className={viewMode === 'grid' ? styles.grid : styles.list}>
          {hospitals?.slice(startIndex, endIndex).map((hospital) => (
            <li key={hospital.id}>
              <DiscoCard hospital={hospital} flag={COUNTRY_FLAGS[hospital?.address?.country ?? ''] || ''} />
            </li>
          ))}
        </ul>
